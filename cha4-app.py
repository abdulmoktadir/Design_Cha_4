...
def render_sidebar_profile_card(container, name, role, institution, image_path, brief_text, full_bio=None, extras=None, tag="Researcher"):
    ...
    if full_bio:
        with container.expander(f"More about {name}", expanded=False):
            st.markdown(f'<div class="sidebar-profile-bio">{escape(full_bio)}</div>', unsafe_allow_html=True)
...
def page_dfs_ahp():
    ...
    num_criteria = st.sidebar.number_input("Number of Criteria", min_value=2, max_value=15, value=9, step=1, key="ahp_n")
    num_scenarios = st.sidebar.number_input("Number of Scenarios", min_value=1, max_value=10, value=1, step=1, key="ahp_s")
    num_experts = st.sidebar.number_input("Number of Experts per Scenario", min_value=1, max_value=8, value=1, step=1, key="ahp_e")
    cr_threshold = st.sidebar.number_input("CR threshold", min_value=0.0, max_value=1.0, value=0.10, step=0.01, key="ahp_thr")

    cr_method = st.sidebar.radio(
    ...
    st.sidebar.divider()
    st.sidebar.subheader("Scenario transition probabilities")
    scen_probs_raw = []
    loaded_probs = []
    loaded_titles = []
    if isinstance(strat_sent_df, pd.DataFrame) and not strat_sent_df.empty:
    ...
    st.sidebar.caption(
        "Normalized scenario probabilities: " + ", ".join([f"S{s+1}={p:.4f}" for s, p in enumerate(scen_probs)])
    )
...
def page_dfs_qfd():
    ...
    n_rc = int(st.sidebar.number_input("Number of RCs", min_value=2, max_value=50, value=9, step=1, key="qfd_nrc"))
    n_ms = int(st.sidebar.number_input("Number of MSs", min_value=2, max_value=50, value=9, step=1, key="qfd_nms"))
    n_exp = int(st.sidebar.number_input("Number of Experts", min_value=1, max_value=10, value=1, step=1, key="qfd_nexp"))
