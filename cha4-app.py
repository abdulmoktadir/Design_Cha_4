...
def render_inventor_profiles_sidebar():
    ...
    render_sidebar_profile_card(
        ...
        brief_text="Research focus: process systems engineering for energy, environment, and sustainability.",
        full_bio=(
        ...
        extras=["Award: 2022 APEC ASPIRE Prize"],
    )
...
def render_delphi_results(clean_scores_df, summary_df, threshold):
    ...
    t1, t2, t3 = st.tabs(["Summary", "Clean Matrix", "Chart"])
...
def delphi_app():
    render_page_header(
        ...
        "Screen, validate, and prioritize criteria from Delphi responses using interval type-2 trapezoidal fuzzy sets."
    )

    c1, c2, c3 = st.columns(3)
    c1.metric("Method", "IT2TrFS-Delphi")
    c2.metric("Input Modes", "Excel + Manual")
    c3.metric("Scoring", "UMF / LMF / Crisp")
            ...
            try:
                ...
                with st.expander("Advanced settings"):
                    ...
                    end_row = st.number_input(
                        ...
                        min_value=1,
                        max_value=max(len(df_raw), 1),
                        value=min(len(df_raw), int(response_block.index.max()) + 2)
                        if not response_block.empty else min(10, len(df_raw)),
                        step=1,
                        key="delphi_end_row"
                    )
...
def cocoso_app():
    render_page_header(
        "📊 IT2TrFS-CoCoSo",
        "Aggregate expert linguistic judgments, normalize benefit-cost criteria, and rank alternatives with CoCoSo indices."
    )

    c1, c2, c3 = st.columns(3)
    c1.metric("Method", "IT2TrFS-CoCoSo")
    c2.metric("Input Type", "Linguistic Expert Ratings")
    c3.metric("Normalization", "Benefit / Cost")

    st.caption("Normalization uses δ⁺ (max) for Benefit and δ⁻ (min) for Cost")

    with st.expander("Linguistic scale (VP…VG)"):
        scale_df = pd.DataFrame(
            [{"Abbr": k, "Meaning": COCOSO_FULL[k], "IT2TrFS": format_it2(v)}
             for k, v in COCOSO_LINGUISTIC_TERMS.items()]
        )
    ...
    if st.button("✅ Run IT2TrFS-CoCoSo", type="primary", use_container_width=True, key="cocoso_run_it2"):
        with st.spinner("Computing..."):
            ...
            t1, t2, t3 = st.tabs(["Final Ranking", "Matrices", "Intermediate Values"])
...
FULL_FORMS = {
    "VLR": "Very Low Relevance", "LR": "Low Relevance", "MR": "Medium Relevance",
    "HR": "High Relevance", "VHR": "Very High Relevance",
    "ELI": "Extremely Low Influence", "VLI": "Very Low Influence",
    "LI": "Low Influence", "MI": "Medium Influence",
    "HI": "High Influence", "VHI": "Very High Influence", "EHI": "Extremely High Influence"
}
...
def wings_app():
    render_page_header(
        ...
        "Model causal interactions, compute total impact and receptivity, and derive normalized importance weights."
    )

    c1, c2, c3 = st.columns(3)
    c1.metric("Method", "IT2TrFS-WINGS")
    c2.metric("Analysis", "Cause–Effect")
    c3.metric("Outputs", "Weights + Report")

    tab_howto, tab_analysis = st.tabs(["📘 How to Use", "📊 Analysis"])

    with tab_howto:
        st.markdown("Use the sidebar to configure components and experts, then run IT2TrFS-WINGS.")
    ...
    with tab_analysis:
        ...
        for e in range(n_experts):
            if len(st.session_state.experts_data[e]["strengths_linguistic"]) != n_components:
                st.session_state.experts_data[e]["strengths_linguistic"] = ["MR"] * n_components
            old_matrix = st.session_state.experts_data[e]["influence_matrix_linguistic"]
            if len(old_matrix) != n_components or any(len(row) != n_components for row in old_matrix):
                st.session_state.experts_data[e]["influence_matrix_linguistic"] = [["ELI"] * n_components for _ in range(n_components)]
        ...
        for e, tab in enumerate(tabs):
            with tab:
                st.subheader(f"Expert {e+1} evaluations")
                ...
                for i in range(n_components):
                    with cols[i % len(cols)]:
                        current = st.session_state.experts_data[e]["strengths_linguistic"][i]
                        term = st.selectbox(
                            component_names[i],
                            options=list(LINGUISTIC_TERMS["strength"].keys()),
                            index=list(LINGUISTIC_TERMS["strength"].keys()).index(current)
                            if current in LINGUISTIC_TERMS["strength"] else 2,
                            key=f"w_str_{e}_{i}"
                        )
                ...
                inf_mat = [[zero_it2() for _ in range(n_components)] for _ in range(n_components)]
                for i in range(n_components):
                    row_cols = st.columns(n_components + 1)
                    row_cols[0].markdown(f"**{component_names[i]} →**")
                    for j in range(n_components):
                        with row_cols[j + 1]:
                            if i == j:
                                ...
                                current = st.session_state.experts_data[e]["influence_matrix_linguistic"][i][j]
                                term = st.selectbox(
                                    f"{component_names[i]}→{component_names[j]}",
                                    options=list(LINGUISTIC_TERMS["influence"].keys()),
                                    index=list(LINGUISTIC_TERMS["influence"].keys()).index(current)
                                    if current in LINGUISTIC_TERMS["influence"] else 0,
                ...
                with st.expander("Flowchart preview"):
                    st.graphviz_chart(
                        generate_flowchart_for_expert(st.session_state.experts_data[e], component_names, expert_idx=e),
                        use_container_width=True
                    )
        ...
        if st.button("🚀 Run IT2TrFS WINGS Analysis", type="primary", use_container_width=True, key="w_run"):
            ...
            t1, t2, t3 = st.tabs(["Results", "Visualization", "Export"])
