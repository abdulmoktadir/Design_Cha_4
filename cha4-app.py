...
# ============================================================
# AUTHENTICATION
@@ -131,9 +230,9 @@ def check_password():
        """
...
@@ -168,13 +267,114 @@ def check_password():
...
def format_it2(it2):
    u, l = it2
    return (
@@ -231,34 +431,19 @@ def scalar_mul_it2(k, A):
    return (new_u, new_l)
...
@@ -267,16 +452,14 @@ def it2_score_components(it2):
    Au, Al = it2
    a, b, c, d, uh1, uh2 = Au
    e, f, g, h, lh1, lh2 = Al
...
@@ -286,16 +469,11 @@ def it2_score_components(it2):
...
@@ -308,7 +486,6 @@ def _safe_div(num, den):
...
@@ -317,10 +494,8 @@ def normalize_it2_matrix_excel(agg_matrix, criteria_types, alternatives, criteri
        ...
        if _is_benefit_type(criteria_types[j]):
            ...
            for alt in alternatives:
@@ -337,25 +512,9 @@ def normalize_it2_matrix_excel(agg_matrix, criteria_types, alternatives, criteri
...
def it2_to_row(it2):
@@ -372,29 +531,18 @@ def format_it2_table(matrix_dict, alternatives, criteria, value_formatter=format
...
def parse_loose_numeric(x):
    if pd.isna(x):
@@ -418,31 +566,18 @@ def build_weight_vector(weight_series):
    return (w / w.sum()).to_numpy(dtype=float)

def aggregate_it2_delphi_from_scores(scores_df):
    """
    scores_df: rows=respondents, cols=criteria, values in {1,2,3,4,5}
    No expert-weight handling is used here. Each valid respondent contributes equally.
    """
    aggregated = {}
    summary_rows = []
...
@@ -452,34 +587,21 @@ def aggregate_it2_delphi_from_scores(scores_df):
        ...
        agg_it2 = (
            (umf_a, umf_b, umf_c, umf_d, uh1, uh2),
            (lmf_e, lmf_f, lmf_g, lmf_h, lh1, lh2),
        )
...
@@ -501,10 +623,8 @@ def detect_delphi_columns(df):
...
@@ -515,163 +635,72 @@ def extract_contiguous_response_block(df, criteria_cols):
                keep_idx.append(idx)
            else:
                break
...
def render_delphi_results(clean_scores_df, summary_df, threshold):
    ...
    display_cols = [
        "Criterion", "Aggregated IT2TrFS", "Mode rating",
        "Score(UMF)", "Score(LMF)", "Crisp score", "Status", "Rank"
    ]
    ...
    st.download_button(
        "⬇️ Download Delphi summary as CSV",
        data=csv_bytes,
        file_name="it2trfs_delphi_summary.csv",
        mime="text/csv",
        use_container_width=True
    )
...
def delphi_app():
    ...
    with tab_excel:
        uploaded_file = st.file_uploader(
            "Upload Delphi response Excel file",
            type=["xlsx", "xls"],
            key="delphi_excel_upload"
        )

        threshold = st.number_input(
            "Acceptance threshold for crisp score",
            min_value=0.0,
            max_value=1.0,
            value=0.60,
            step=0.01,
            key="delphi_threshold_excel"
        )
            ...
            try:
                ...
                with st.expander("Advanced settings"):
                    criteria_cols = st.multiselect(
                        "Detected Delphi criteria columns",
                        options=list(df_raw.columns),
                        default=detected_cols,
                        key="delphi_criteria_cols"
                    )

                    start_row = st.number_input(
                        "Start row of response block (1-based, after header)",
                        min_value=1,
                        max_value=max(len(df_raw), 1),
                        value=1 if response_block.empty else int(response_block.index.min()) + 2,
                        step=1,
                        key="delphi_start_row"
                    )
                    end_row = st.number_input(
                        "End row of response block (1-based, after header)",
                        min_value=1,
                        max_value=max(len(df_raw), 1),
                        value=min(len(df_raw), int(response_block.index.max()) + 2) if not response_block.empty else min(10, len(df_raw)),
                        step=1,
                        key="delphi_end_row"
                    )
...
@@ -680,121 +709,71 @@ def delphi_app():
    ...
    with tab_manual:
        ...
        threshold_manual = st.number_input(
            "Acceptance threshold",
            min_value=0.0,
            max_value=1.0,
            value=0.60,
            step=0.01,
            key="delphi_threshold_manual"
        )
        ...
        edited_scores = st.data_editor(
            st.session_state.delphi_manual_df,
            use_container_width=True,
            column_config={
                c: st.column_config.NumberColumn(c, min_value=1, max_value=5, step=1, format="%d")
                for c in criteria_names
            },
            key="delphi_manual_editor"
        )
...
def identity_it2(n):
    ...
    for i in range(n):
@@ -994,7 +890,6 @@ def identity_it2(n):
...
def compute_total_relation_matrix(normalized_matrix):
    ...
    for i in range(n):
        for j in range(n):
@@ -1003,7 +898,6 @@ def compute_total_relation_matrix(normalized_matrix):
            Z_4d[0, 1, i, j, :2] = Au[4:]
            Z_4d[1, 0, i, j, :] = Al[:4]
            Z_4d[1, 1, i, j, :2] = Al[4:]
    ...
    for i in range(2):
        for j in range(2):
            if j == 0:
@@ -1014,140 +908,66 @@ def compute_total_relation_matrix(normalized_matrix):
...
def wings_method_experts(strengths_list, influence_matrices_list, weights=None):
    ...
    # 1. Aggregate experts into average SIDRM
    ...
    # 3. Normalized direct-relation matrix Z
    Z_mat = [[zero_it2() for _ in range(n)] for _ in range(n)]
    for i in range(n):
        for j in range(n):
            Au, Al = avg_sidrm[i][j]
            new_u = (
                Au[0] / s if s else 0.0,
                Au[1] / s if s else 0.0,
                Au[2] / s if s else 0.0,
                Au[3] / s if s else 0.0,
                Au[4], Au[5]
            )
            new_l = (
                Al[0] / s if s else 0.0,
                Al[1] / s if s else 0.0,
                Al[2] / s if s else 0.0,
                Al[3] / s if s else 0.0,
                Al[4], Al[5]
            )
    ...
    # 6. TI+TR and TI-TR
    engagement = [add_it2(TI[i], TR[i]) for i in range(n)]   # TI + TR
    role = [sub_it2(TI[i], TR[i]) for i in range(n)]         # TI - TR
...
@@ -1157,58 +977,39 @@ def format_it2_df(mat, index, columns):
    return df
...
def create_word_report(results, component_names, n_experts=1, expert_weights=None):
    ...
    if expert_weights and n_experts > 1:
        weights_text = "Expert weights: " + ", ".join(
            [f"Expert {i+1}: {weight:.4f}" for i, weight in enumerate(expert_weights)]
        )
    ...
    for i, name in enumerate(component_names):
        ...
        row[0].text = name
@@ -1218,136 +1019,70 @@ def create_word_report(results, component_names, n_experts=1, expert_weights=Non
...
def get_word_download_link(doc):
    ...
    href = (
        ...
        f'base64,{b64}" download="it2trfs_wings_analysis_report.docx">Download Word Report</a>'
    )
...
def wings_app():
    ...
    with tab_analysis:
        ...
        stale_expert_keys = [
            k for k in st.session_state.experts_data.keys()
            if isinstance(k, int) and k >= n_experts
        ]
...
@@ -1361,226 +1096,131 @@ def wings_app():
                            val = "ELI"
                        new_row.append(val)
                refreshed_infl.append(new_row)

            st.session_state.experts_data[e] = {
                "strengths_linguistic": refreshed_strengths,
                "influence_matrix_linguistic": refreshed_infl,
            }
        ...
        for e in range(n_experts):
            with tabs[e]:
                ...
                for i in range(n_components):
                    with cols[i]:
                        cur = (
                            st.session_state.experts_data[e]["strengths_linguistic"][i]
                            if i < len(st.session_state.experts_data[e]["strengths_linguistic"])
                            else "HR"
                        )
                        term = st.selectbox(
                            component_names[i],
                    ...
                    for j in range(n_components):
                        with row_cols[j + 1]:
                            if i == j:
                                ...
                                cur = (
                                    st.session_state.experts_data[e]["influence_matrix_linguistic"][i][j]
                                    if i < len(st.session_state.experts_data[e]["influence_matrix_linguistic"])
                                    and j < len(st.session_state.experts_data[e]["influence_matrix_linguistic"][i])
                                    else "ELI"
                                )
        ...
        if st.button("✅ Run IT2TrFS-WINGS", type="primary", use_container_width=True):
            ...
            with t1:
                ...
                st.dataframe(
                    format_it2_df(results["average_sidrm"], component_names, component_names),
                    use_container_width=True
                )
                ...
                st.dataframe(
                    format_it2_df(results["normalized_matrix"], component_names, component_names),
                    use_container_width=True
                )
                ...
                st.dataframe(
                    format_it2_df(results["total_matrix"], component_names, component_names),
                    use_container_width=True
                )

                results_df = pd.DataFrame({
                    "Component": component_names,
                    "TI (IT2TrFS)": [format_it2(results["total_impact"][i]) for i in range(len(component_names))],
                    "TR (IT2TrFS)": [format_it2(results["total_receptivity"][i]) for i in range(len(component_names))],
                    "TI+TR (IT2TrFS)": [format_it2(results["engagement"][i]) for i in range(len(component_names))],
                    "TI-TR (IT2TrFS)": [format_it2(results["role"][i]) for i in range(len(component_names))],
                ...
                classification_df = pd.DataFrame({
                    "Component": component_names,
                    "Type": ["Cause" if results["role_defuzz"][i] > 0 else "Effect" for i in range(len(component_names))],
                    "TI-TR": results["role_defuzz"],
                    "TI+TR": results["engagement_defuzz"],
                    "E(wj)": results["Ew"],
                    "Normalized Weight": results["normalized_weights"],
            ...
            with t2:
                ...
                st.dataframe(
                    weight_df.style.format({"Normalized Weight": "{:.6f}"}),
                    use_container_width=True,
                    hide_index=True
                )
...
# =========================================================
# MAIN NAVIGATION (THREE MODULES)
# =========================================================

# ============================================================
# MAIN NAVIGATION (FOUR MODULES)
# ============================================================
def main():
    ...
    page = st.sidebar.radio("Navigate", ["IT2TrFS-Delphi", "IT2TrFS-WINGS", "IT2TrFS-CoCoSo", "👥 Researcher Profiles"])
    st.sidebar.markdown("---")
    ...
    if page == "IT2TrFS-Delphi":
        ...
        st.markdown(
            ...
            <div style="text-align: center; margin-bottom: 1rem;">
                ...
                <p style="color: #475569;">This toolkit is developed and maintained by the following researchers.</p>
            </div>
            """,
            unsafe_allow_html=True,
        )
        ...
        st.markdown(
            ...
            <div style="text-align: center; margin: 2rem 0 1rem;">
                <p style="color: #64748b;">For questions or collaboration, please contact the corresponding author.</p>
            </div>
            """,
            unsafe_allow_html=True,
        )
